...
func TestMaxNumberPriorityQueue_Pop(t *testing.T) {
	compare := func(i, j interface{}) bool {
		return i.(int) > j.(int)
	}
	...
	priorityQueue.Push(500, 200)

	element, err := priorityQueue.Pop()
	assert.Nil(t, err)
	assert.Equal(t, priorityQueue.Size(), 2)
	assert.Equal(t, element, 500)

	element, err = priorityQueue.Pop()
	assert.Nil(t, err)
	assert.Equal(t, priorityQueue.Size(), 1)
	assert.Equal(t, element, 200)

	element, err = priorityQueue.Pop()
	assert.Nil(t, err)
	assert.Equal(t, priorityQueue.Size(), 0)
	assert.Equal(t, element, 100)
